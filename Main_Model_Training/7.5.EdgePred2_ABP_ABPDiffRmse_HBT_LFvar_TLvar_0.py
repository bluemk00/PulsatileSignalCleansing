# added one artifact type to 6.1.3 (no overlap)
...
from tensorflow.keras.layers import Input, Dropout, Dense,Concatenate, Permute, Activation, MaxPooling2D, Conv2D, Flatten,Dot,Reshape,MaxPooling1D,Conv1D, GaussianNoise, LayerNormalization,UpSampling1D, Conv1DTranspose, GlobalAveragePooling2D, LSTM, Bidirectional, GaussianNoise, TimeDistributed, GRU, Bidirectional
import tensorflow as tf
...
class DataBatch(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # High frequency artifact
        SelectedBatch[NoiseBatchInd%10==4] = self.IPP(SelectedBatch[NoiseBatchInd%10==4])
        ...
        SelectedBatch[mask] = self.HFNoise(SelectedBatch[mask])
        SelectedBatch[NoiseBatchInd>29] = self.TimeLagVar(SelectedBatch[NoiseBatchInd>29])
...
#         OutMean = OutAmplitude[:, 0:1]
#         OutAmplitude = OutAmplitude[:, 1:]
...
#         OutHBT = tf.complex(tf.cast(OutpData, tf.float64), tf.cast(shtr, tf.float64))
        
        return (InpData), (OutpData, OutpData_diff, shtr) # (InpData), (OutpData) 
...
@tf.function
def MSEtop(y_true, y_pred):
    topK = 10
    SE = (y_true - y_pred)**2
    TopSE = tf.nn.top_k(SE, topK)[0]
    SSE = tf.reduce_sum(TopSE, axis=-1)
    
    return 0.1*tf.reduce_mean(SSE)

@tf.function
def Amplitudetop(y_true, y_pred):
    topK = 10
    SE = (y_true - y_pred)**2
    TopSE = tf.nn.top_k(SE, topK)[0]
    SSE = tf.reduce_sum(TopSE, axis=-1)
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        FrameSize = 50
        OverlapSize = 50 # actually, frame interval
...
#         MeanOut = Reshape((-1,),name='MeanOut')(AmplitudeOut[:,0:1])
#         AmplitudeOut= Reshape((-1,),name='AmplitudeOut')(AmplitudeOut[:, 1:])
...
#         HBTOut = tf.complex(tf.cast(ABPOut, tf.float64), tf.cast(shtr, tf.float64))
#         HBTOut = Reshape((-1,),name='HBTOut')(HBTOut)

        AEModel = Model(InpL, (ABPOut, ABPOut_diff, HBTOut))

        # AEModel.load_weights('../4.TrainedModel/7.0_depth50_overlap40/7.0.EdgePred2_2617_AMP0.00034_valAMP0.00028_ABPd0.00486_valABPd0.00393_ABP0.00053_valABP0.00047.hdf5')
        ...
        SaveFilePath = '../4.TrainedModel/7.5_depth50_overlap0/7.1.2.EdgePred2_{epoch:d}_ABP{ABPOut_loss:.5f}_valABP{val_ABPOut_loss:.5f}_ABPd{ABPOut_diff_loss:.5f}_valABPd{val_ABPOut_diff_loss:.5f}_HBT{HBTOut_loss:.5f}_valHBT{val_HBTOut_loss:.5f}.hdf5'
        ...
        AEModel.compile(loss={'ABPOut':'mse','ABPOut_diff':RMSE,'HBTOut':DistCost}, optimizer=adam) #loss={'AmplitudeOut':AmplitudeCost,'ABPOut':'mse'}
