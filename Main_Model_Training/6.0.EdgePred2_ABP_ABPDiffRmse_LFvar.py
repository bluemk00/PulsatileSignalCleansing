...
class DataBatch(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        NoiseBatchSize = int(len(BatchIndex)*0.6)
        
        # Batch Selection
        SelectedBatch = self.DataInp[BatchIndex]
        NoiseSelectedBatch = SelectedBatch[:NoiseBatchSize].copy()
        NormalSelectedBatch = SelectedBatch[NoiseBatchSize:].copy()
        
        # Noise sampling index 생성
        #NoiseBatchInd = np.random.choice( 5, NoiseBatchSize,  replace=True)
        Noise1Size = int(NoiseBatchSize * 0.2)
        Noise2Size = int(NoiseBatchSize * 0.2)
        Noise3Size = int(NoiseBatchSize * 0.2)
        Noise4Size = int(NoiseBatchSize * 0.2)
        Noise5Size = int(NoiseBatchSize * 0.2)
        NoiseBatchInd = np.concatenate([np.ones(i)*(num) for num, i in enumerate([Noise1Size,Noise2Size,Noise3Size,Noise4Size,Noise5Size])]) 
        NoiseBatchInd = np.random.permutation(NoiseBatchInd)

        # select for saturation to ABP maximum
        NoiseSelectedBatch[NoiseBatchInd==0] = self.SatABPmax(NoiseSelectedBatch[NoiseBatchInd==0])

        # select for saturation to ABP minmum
        NoiseSelectedBatch[NoiseBatchInd==1] = self.SatABPmin(NoiseSelectedBatch[NoiseBatchInd==1])

        # Reduced pulse pressure artifact
        NoiseSelectedBatch[NoiseBatchInd==2] = self.RPP(NoiseSelectedBatch[NoiseBatchInd==2])

        # High frequency artifact
        NoiseSelectedBatch[NoiseBatchInd==3] = self.HFNoise(NoiseSelectedBatch[NoiseBatchInd==3])

        # Impulse artifact
        NoiseSelectedBatch[NoiseBatchInd==4] = self.ImpNoise(NoiseSelectedBatch[NoiseBatchInd==4])

        InpData = np.concatenate([NoiseSelectedBatch, NormalSelectedBatch],axis=0)
        OutpData = np.concatenate([SelectedBatch[:NoiseBatchSize, -500:], NormalSelectedBatch[:, -500:]],axis=0)
        ...
        NoiseBatchInd = np.array([int(ind+10) if num%2==1 else int(ind) for num, ind in enumerate(NoiseBatchInd)])
        NoiseBatchInd = np.random.permutation(NoiseBatchInd)
...
#         OutAmplitude = tf.abs(Outfft)*(1/(Outfft.shape[-1])) # 상대적 진폭 Relative amplitude
        
        return (InpData), (OutpData, OutpData_diff) # (InpData), (OutpData) 
    ...
    def SatABPmax(self,SatABPmaxSet):
        ...
        SatABPmaxSet[:, 5500:] = VecToSat[:, SliceStart:SliceEnd]
    ...
    def SatABPmin(self,SatABPminSet):
        ...
        SatABPminSet[:, 5500:] = VecToSat[:, SliceStart:SliceEnd]
    ...
    def HFNoise (self,HFASet):
        ...
        # for left edge
        LeftNoisedIndNb = np.random.randint(0, 55, 1)
        LeftNoisedInd = np.sort(random.sample(range(5500), 2*LeftNoisedIndNb[0]))
        ...
        if len(LeftNoisedInd) > 0:
            LeftNoisedInd = np.concatenate(np.split(np.arange(5500), LeftNoisedInd.ravel())[1::2]).copy()
            HFASet[:,LeftNoisedInd] += np.random.normal(0, 0.05, HFASet[:,LeftNoisedInd].shape)
            HFASet[:,LeftNoisedInd] = np.clip(HFASet[:,LeftNoisedInd], 0.05, 0.95)

        # for right edge
        RightNoisedIndNb = np.random.randint(1, 11, 1)
        RightNoisedInd = np.sort([5500]+random.sample(range(5500, 6000), RightNoisedIndNb[0])*2+[6000])
        CandInd = np.reshape(RightNoisedInd, (-1,2))
        SecDiff = CandInd[:,1] - CandInd[:,0] 
        RightNoisedInd = CandInd[(SecDiff <= MaxDura) & (SecDiff >= RightMinDura)]

        if len(RightNoisedInd) < 1:
            HFASet[:,range(5500, 6000)] += np.random.normal(0, 0.05, HFASet[:,range(5500, 6000)].shape)
            HFASet[:,range(5500, 6000)] = np.clip(HFASet[:,range(5500, 6000)], 0.05, 0.95)
        else:
            RightNoisedInd = np.concatenate(np.split(np.arange(6000), RightNoisedInd.ravel())[1::2]).copy()
            HFASet[:,RightNoisedInd] += np.random.normal(0, 0.05, HFASet[:,RightNoisedInd].shape)
            HFASet[:,RightNoisedInd] = np.clip(HFASet[:,RightNoisedInd], 0.05, 0.95)
    ...
    def LFVar (self,LFVSet):
        ...
        partition_idx = np.sort(random.sample(range(6000), np.random.randint(1,6,1)[0]))
        # end points of intervals
        ext_partition_idx = np.concatenate([[0], partition_idx, [6000]])
        ...
        LFVSet = image * LFVSet
        LFVSet = np.clip(LFVSet, 0.05, 0.95)
...
if __name__ == "__main__":
    ...
    ## Data selection
    TrSet = np.load('hp_det_res_art_60s_normed_train.npy')[:] 
    ValSet = np.load('hp_det_res_art_60s_normed_valid.npy')[:] 
    
    print('Training set total N size',TrSet.shape[0])
...
#         AmplitudeOut = tf.abs(fft)*(1/(fft.shape[-1])) 
#         AmplitudeOut= Reshape((-1,),name='AmplitudeOut')(AmplitudeOut)

        AEModel = Model(InpL, (ABPOut, ABPOut_diff))
        # AEModel.load_weights('./6.EdgePred2_ABP_ABPDiffRmse_LFvar/EdgePred2_197_0.00242_0.00344_0.00021_0.00032.hdf5')
        ...
        SaveFilePath = './6.org/6.0.EdgePred2_{epoch:d}_{ABPOut_diff_loss:.5f}_{val_ABPOut_diff_loss:.5f}_{ABPOut_loss:.5f}_{val_ABPOut_loss:.5f}.hdf5'
        ...
        earlystopper = EarlyStopping(monitor='val_loss', patience=5000, verbose=1,restore_best_weights=True)
        history = LossHistory()
        ...
        AEModel.compile(loss={'ABPOut_diff':RMSE,'ABPOut':'mse'}, optimizer=adam) #loss={'AmplitudeOut':AmplitudeCost,'ABPOut':'mse'}

        AEModel.fit(TrainSet, validation_data = (ValidSet), verbose=1, epochs=1500, callbacks=[history,earlystopper]) #class_weight=class_weight
