# added one artifact type to 6.2
...
class DataBatch(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        NoiseBatchInd = np.array([int(ind+10) if num%3==1 else int(ind+20) if num%3==2 else int(ind) for num, ind in enumerate(NoiseBatchInd)])
        NoiseBatchInd = np.random.permutation(NoiseBatchInd)
        ...
        SelectedBatch[NoiseBatchInd>19] = self.HFNoise(SelectedBatch[NoiseBatchInd>19])
...
#         print('1',OutAmplitude.shape)
        ...
        return (InpData), (OutpData, OutAmplitude) # (InpData), (OutpData) 
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        # AEModel.load_weights('../4.TrainedModel/final_7.0.1_depth50_overlap40/7.0.1.EdgePred2_2942_AMP0.00028_valAMP0.00047_ABP0.00045_valABP0.00077.hdf5')
        ...
        SaveFilePath = '../4.TrainedModel/final_ver2_7.0.1_depth50_overlap40/AE_ABPAMP_{epoch:04}_val{val_loss:.7f}_valABP{val_ABPOut_loss:.7f}_valAMP{val_AmplitudeOut_loss:.7f}.hdf5'
        ...
        AEModel.compile(loss={'AmplitudeOut':AmplitudeCost,'ABPOut':'mse'}, optimizer=adam) #loss={'AmplitudeOut':AmplitudeCost,'ABPOut':'mse'}
