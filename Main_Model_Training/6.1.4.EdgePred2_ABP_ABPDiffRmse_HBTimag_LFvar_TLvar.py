...
#         shtr = tf.math.real(tf.signal.ifft(sfhtr))
        ...
        def Hilbert (sftr):
            sfhtr = tf.complex(0.0,-1.0) * tf.cast(tf.math.sign(sffr), tf.complex64) * sftr
            return tf.math.real(tf.signal.ifft(sfhtr))
        
        HBTOut = tf.keras.layers.Lambda(Hilbert)(sftr)
        HBTOut = Reshape((-1,),name='HBTOut')(HBTOut)
#         HBTimagOut = tf.math.imag(HBTOut)
#         HBTimagOut = Reshape((-1,),name='HBTimagOut')(HBTimagOut)
#         HBTOut = tf.complex(tf.cast(ABPOut, tf.float64), tf.cast(shtr, tf.float64))
#         HBTOut = Reshape((-1,),name='HBTOut')(HBTOut)

        AEModel = Model(InpL, (ABPOut, ABPOut_diff, HBTOut))
#         AEModel.load_weights('./6.1.4.EdgePred2_ABP_ABPDiffRmse_HBTph_LFvar_TLvar/EdgePred2_518_0.00355_0.00435_0.00037_0.00047.hdf5')
...
#             AD = 1-(tf.math.real(y_true)*tf.math.real(y_pred)+(tf.math.imag(y_true)*tf.math.imag(y_pred)))/2
#             AD = tf.abs(y_true - y_pred)
            AD = 0.01 * (tf.math.real(y_true)-tf.math.real(y_pred))**2 + (tf.math.imag(y_true)-tf.math.imag(y_pred))**2
        ...
        SaveFilePath = '../4.TrainedModel/6.1.4_depth50_overlap40/6.1.4.EdgePred2_{epoch:d}_ABP{ABPOut_loss:.5f}_valABP{val_ABPOut_loss:.5f}_ABPd{ABPOut_diff_loss:.5f}_valABPd{val_ABPOut_diff_loss:.5f}_HBT{HBTOut_loss:.5f}_valHBT{val_HBTOut_loss:.5f}.hdf5'
