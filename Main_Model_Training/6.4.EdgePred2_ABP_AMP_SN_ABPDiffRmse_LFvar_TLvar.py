...
#         print('1',OutAmplitude.shape)
        ...
        ### FFT normal complex number (Sign)
        OutSign = tf.math.sign(Outfft)
        
        return (InpData), (OutpData, OutpData_diff, OutAmplitude, OutSign) # (InpData), (OutpData) 
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        SignOut = Reshape((-1,),name='SignOut')(tf.math.sign(fft))

        AEModel = Model(InpL, (ABPOut, ABPOut_diff, AmplitudeOut, SignOut))
#         AEModel.load_weights('./6.4.EdgePred2_ABP_AMP_SN_ABPDiffRmse_LFvar_TLvar/EdgePred2_197_0.00242_0.00344_0.00021_0.00032.hdf5')
        ...
        def AngleDistCost(y_true, y_pred):
            AD = tf.abs(y_true - y_pred) / 100.0
            MAD = tf.reduce_mean(AD)
        ...
        SaveFilePath = '../4.TrainedModel/6.4_depth50_overlap40/6.4.EdgePred2_{epoch:d}_Sn{SignOut_loss:.5f}_valSn{val_SignOut_loss:.5f}_AMP{AmplitudeOut_loss:.5f}_valAMP{val_AmplitudeOut_loss:.5f}_ABPd{ABPOut_diff_loss:.5f}_valABPd{val_ABPOut_diff_loss:.5f}_ABP{ABPOut_loss:.5f}_valABP{val_ABPOut_loss:.5f}.hdf5'
        ...
        AEModel.compile(loss={'SignOut':AngleDistCost,'AmplitudeOut':AmplitudeCost,'ABPOut_diff':RMSE,'ABPOut':'mse'}, optimizer=adam) #loss={'AmplitudeOut':AmplitudeCost,'ABPOut':'mse'}
